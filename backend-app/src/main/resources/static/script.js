...
function initCartPage() {
  ...
  list.innerHTML = booking.passengers
    .map(
      (p) =>
        `<div class="ticket-passenger-row"><span>${escapeHtml(p.name || "Passenger")}</span><span>Seat ${p.seatNumber}</span><span>${escapeHtml(formatCurrency(getPassengerPrice(booking, p)))}</span></div>`
    )
    .join("");
  total.innerHTML = `<h2>Total: ${escapeHtml(formatCurrency(booking.total))}</h2>`;
...
async function initPaymentPage() {
  ...
  summary.innerHTML = `
    ...
    <p><strong>Total:</strong> ${escapeHtml(formatTryCurrency(total))}</p>
  ...
  const setPaymentFormBusy = (busy) => {
    paymentInProgress = busy;
    if (submitButton) {
      submitButton.disabled = busy;
      submitButton.textContent = busy ? "Processing payment..." : "Complete Payment";
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (!paymentAuthorized) {
        ...
        showToast("Payment was approved, but ticket creation failed. Please do not pay again; check your profile or contact support.", "error");
      }
    }
  });
}
...
function initForgotPasswordFlow({ toggleId, formId, emailId, resultId, role }) {
  ...
  const submitButton = form.querySelector('button[type="submit"]');
  const setForgotPasswordBusy = (busy) => {
...
function formatCurrency(value, currency = "TRY") {
  ...
  if (currency === "TRY") {
    return formatTryCurrency(safeNumber);
  }
